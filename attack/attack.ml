...
let equal a b =
  ...
  try
    for i = 0 to pred ln do
      if not (Char.equal a.[i] b.[i]) then raise_notrace Diff;
      Unix.sleepf 0.0001
    done;
...
let rec run hash prefix =
  ...
  for i = 0 to pred (Array.length range) do
  ...
  let results =
    List.sort
      (fun (_, v0) (_, v1) -> (compare : int -> int -> int) v1 v0)
      results
  in

  match results with
  | [] -> assert false
  | (hit, _) :: _ -> if String.length hit = length then hit else run hash hit
