...
let test_ccea fn_0 fn_1 =
  ...
  let m1 = Benchmark.run fn_1 in
  match
    ( Linear_algebra.ols (fun m -> m.(1)) [| (fun m -> m.(0)) |] m0,
      Linear_algebra.ols (fun m -> m.(1)) [| (fun m -> m.(0)) |] m1 )
  with
  | Ok (estimates_0, r_square_0), Ok (estimates_1, r_square_1) ->
      Fmt.epr "> Calculating Z.\n%!";
      let z =
        (estimates_0.(0) -. estimates_1.(0))
        /. sqrt ((r_square_0 ** 2.) +. (r_square_1 ** 2.))
      in
      Ok z
...
let ccea ~name_of_fns_0 ~name_of_fns_1 fns_0 fns_1 =
  ...
  | Ok eqaf, Ok stdlib -> Ok (eqaf, stdlib)
  | Error (`Msg err), Ok _ ->
...
struct
  ...
  let last_chance () =
    ...
    match
      ccea ~name_of_fns_0:eqaf_name ~name_of_fns_1:stdlib_name
        (V eqaf_true, V eqaf_false)
        (V stdlib_true, V stdlib_false)
    with
    | Error () -> exit_failure
    | Ok (eqaf, stdlib) ->
        if eqaf >= -30. && eqaf <= 30. then (
          Fmt.pr "Z¹ = %f, Z² = %f.\n%!" eqaf stdlib;
          exit_success)
        else (
          Fmt.pr "Z¹ = %f, Z² = %f.\n%!" eqaf stdlib;
  ...
  let test () =
    let open Benchmark in
    match
      spss ~name_of_fns_0:eqaf_name ~name_of_fns_1:stdlib_name
        (V eqaf_true, V eqaf_false)
        (V stdlib_true, V stdlib_false)
    with
    | Error () -> last_chance ()
